...
namespace PresentationLayer.ConsoleUI.UI
{
    class CLI : IUserInterface
    {
        ...
        protected void StartTest()
        {
            ...
            testContent.ToList().ForEach(question => testItems.Add(GetAnswer(question)));
            ...
            {
                Console.WriteLine("Difficulty: " + question.Difficulty.Name);
                Console.WriteLine(question.Text);
                for (int index = 0; index < question.Answers.Count; index++)
                {
                    Console.WriteLine($"{index + 1}: {question.Answers[index].Text}");
                }
                while (true)
                {
                    if (int.TryParse(Console.ReadLine(), out int input) && input <= 4 && input >= 1)
                    {
                        return new TestItemModel(0, question.Answers[input - 1].IsCorrect, question);
                    }
                }
            }
        }
